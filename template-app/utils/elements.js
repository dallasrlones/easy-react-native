...
elements.A = ({ children, style, screen, navigation, onPress }) => {
    ...
    return <TouchableOpacity style={[styles.a, style]} onPress={() => { navigation.navigate(screen) }}>{children}</TouchableOpacity>
};
...
elements.HSPLIT = ({ children }) => {
    ...
    if (!validateColWidths(Children.toArray(children))) {
        return null;
    }

    return (
        <View style={hSplitStyles.row}>
...
elements.ICON = ({ name, size, height, width, color, backgroundColor, borderRadius, onPress, style }) => {
    return (
        <TouchableOpacity onPress={onPress} style={{ ...style, height, width }}>
            <Text style={{ height, width, textAlign: 'center', textAlignVertical: 'center' }}>
                {/* <Ionicons name={name} size={size} borderRadius={borderRadius} color={color} style={{ height, width }} /> */}
                <AntDesign name={name} size={size} color={color} backgroundColor={backgroundColor} borderRadius={borderRadius} />
            </Text>
        </TouchableOpacity>
    )
}
